"""빌드 전 훅 — assets/efb_hangul32.bin 을 PROGMEM C++ 배열로 굽는다.

생성물(node/src/font_data.cpp)은 ~7MB 소스라 git 에 넣지 않는다(.gitignore).
바이너리(1.4MB)만 커밋하고 빌드 때마다 여기서 만든다 — 둘이 어긋날 일이 없다.

폰트 원본: 나눔고딕코딩 (SIL OFL 1.1) — assets/OFL.txt. tools/gen_font.py 로 .bin 생성.
"""
...
BIN = PROJECT.parent / "assets" / "efb_hangul32.bin"
...
def main() -> None:
    ...
    lines = [
        ...
        "// 원본: assets/efb_hangul32.bin (나눔고딕코딩, SIL OFL 1.1 — assets/OFL.txt)",
        "//",
        "// 32px 1bpp, 글립당 128B 고정. ASCII 95자 + 완성형 한글 11,172자.",
        ...
        "",
        "const uint8_t EFB_HANGUL32[] PROGMEM = {",
    ]
    for i in range(0, len(data), 16):
        lines.append("    " + ",".join(f"0x{b:02X}" for b in data[i : i + 16]) + ",")
    lines += [
        "};",
        f"extern const size_t EFB_HANGUL32_LEN = {len(data)};",
        "",
    ]
