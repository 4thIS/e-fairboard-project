...
#
# font_size 는 실제 px 높이다. 노드가 상용한글 2,350(KS X 1001) 을 32/48/64px 로 각각 구워
# (native 비트맵) 확대 없이 그린다 (폰트 렌더 V2, HANDOFF_FONT_RENDER_V2.md). 세 크기만 쓴다.
# 현재 노드(16px 정수배)에서도 32/48/64 는 16의 배수라 같은 픽셀크기로 그려져 호환된다.
#
# 12.48" e-Paper 1304×984 기준 (팀 소개는 패널을 세워 984×1304). 7.5"(800×480)에서 이관.
# 폰트: 제목/헤더/강조 64px(큰), 본문 48px(중간), 32px(작은)은 예약. 대형 디스플레이용.
...
TEMPLATES: dict[int, TemplateDef] = {
    0: TemplateDef(0, "행사 안내", (
        FieldDef(0, "제목", 48, 48, 64, 57),
        FieldDef(1, "일시", 48, 240, 48, 78),
        FieldDef(2, "장소", 48, 340, 48, 78),
        FieldDef(3, "비고", 48, 440, 48, 78),
    ), QrDef(968, 648, 288)),
    1: TemplateDef(1, "부스 지도", (
        FieldDef(0, "구역명", 48, 100, 64, 57),
        FieldDef(1, "부스번호", 48, 360, 64, 57),
    ), QrDef(968, 648, 288)),
    2: TemplateDef(2, "모집 공고", (
        FieldDef(0, "제목", 48, 48, 64, 57),
        FieldDef(1, "마감", 48, 240, 48, 78),
        FieldDef(2, "대상", 48, 340, 48, 78),
    ), QrDef(968, 648, 288)),
    3: TemplateDef(3, "일정표", (
        FieldDef(0, "날짜", 48, 48, 64, 57),
        FieldDef(1, "세션1", 48, 240, 48, 78),
        FieldDef(2, "세션2", 48, 340, 48, 78),
        FieldDef(3, "세션3", 48, 440, 48, 78),
    ), QrDef(968, 648, 288)),
    # 세로 팜플렛 — 패널을 세워 984×1304. 상세 소개는 QR 너머 웹페이지.
    4: TemplateDef(4, "팀 소개", (
        FieldDef(0, "팀명", 48, 64, 64, 42),
        FieldDef(1, "주제1", 48, 300, 48, 57),
        FieldDef(2, "주제2", 48, 400, 48, 57),
        FieldDef(3, "주제3", 48, 500, 48, 57),
